...
class TestScheduleJobViews(AdminXTestCase):
    ...
    def test_status_endpoint(self):
        resp = self.client.get("/api/v1/webservice/jobs/status/")
        self.assert_ok(resp)

    def test_reload_endpoint(self):
        resp = self.client.post("/api/v1/webservice/jobs/reload/")
        self.assert_ok(resp)
    ...
    def test_run_once_with_result(self):
        ...
        job = ScheduleJob.objects.create(
            name="RunTest2", handler="djangoadminx.webservice.tasks.sample_task",
    ...
    def test_run_once_invalid_job(self):
        resp = self.client.post("/api/v1/webservice/jobs/00000000-0000-0000-0000-000000000000/run_once/")
        self.assert_fail(resp, 404)

    def test_list_job_logs(self):
        resp = self.client.get("/api/v1/webservice/job-logs/")
        self.assert_ok(resp)


class TestTasks:
    def test_sample_task(self):
        from djangoadminx.webservice.tasks import sample_task
        assert sample_task() == "ok"

    @override_settings()
    def test_system_resource_monitor(self):
        from djangoadminx.webservice.tasks import system_resource_monitor
    ...
    def test_ntp_sync_can_run(self):
        ...
        from djangoadminx.webservice.tasks import ntp_sync
